...
class _Algo {
  ...
  /// This determines if the Shahanshahi (Persian) year is
  /// leap (366-day long) or is the common year (365 days), and
  /// finds the day in March (Gregorian calendar) of the first
  /// day of the Shahanshahi year (jy).
  ///
  /// [1. see here](http://www.astro.uni.torun.pl/~kb/Papers/EMP/PersianC-EMP.htm)
  ///
  /// [2. see here](http://www.fourmilab.ch/documents/calendar/)
  static _ShahanshahiCalculation calculate(int sy) {
    // Shahanshahi years starting the 33-year rule.
    ...
    final  jy = sy - 1180; // convert Shahanshahi to Jalali
    ...
    // Find the limiting years for the Shahanshahi year jy.
    ...
    // of the current Shahanshahi year in the Persian calendar.
    ...
    return _ShahanshahiCalculation(leap: leap, gy: gy, march: march);
  }

  /// create from julian day number
  static Shahanshahi createFromJulianDayNumber(
    int julianDayNumber,
    int hour,
    int minute,
    int second,
    ...
    if (k >= 0) {
      ...
      // Previous Shahanshahi year.
      jy -= 1;
      k += 179;
      if (r.leap == 1) k += 1;

      // since we changed Shahanshahi year to previous
  ...
  static Shahanshahi createFromYearMonthDay(
    int year,
    int month,
    int day,
    int hour,
    ...
    // should be between: Shahanshahi(-61, 1, 1) and Shahanshahi(3177, 10, 11)
    if (year < -61 || year > 3177) {
      throw DateException('Shahanshahi date is out of computable range.');
    }

    // check month
    if (month < 1 || month > 12) {
      throw DateException('Shahanshahi month is out of valid range.');
    }

    // check lower bound of day
    if (day < 1) {
      throw DateException('Shahanshahi day is out of valid range.');
    }
...
/// Internal used for Shahanshahi calculation
class _ShahanshahiCalculation {
  /// Number of years since the last leap year
  /// (0 (inclusive) to 4 (exclusive))
  final int leap;

  /// Gregorian year of the beginning of Shahanshahi year
  final int gy;

  /// The March day of Farvardin the 1st (1st day of jy)
  final int march;

  const _ShahanshahiCalculation({
    required this.leap,
    required this.gy,
    required this.march,
  });
